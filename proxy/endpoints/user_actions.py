...
from proxy.mapping.scene import format_jellyfin_item
from proxy.stash.client import stash_query
from proxy.stash.tags import get_or_create_tag
...
async def endpoint_user_favorites(request):
    ...
    try:
        tag_id = get_or_create_tag(runtime.FAVORITE_TAG)
        if not tag_id:
            return JSONResponse({"Items": [], "TotalRecordCount": 0, "StartIndex": 0})
...
def _toggle_scene_favorite(item_id: str, add: bool) -> None:
    numeric_id = item_id.replace("scene-", "")
    tag_id = get_or_create_tag(runtime.FAVORITE_TAG)
    if not tag_id:
        return
    scene_res = stash_query(
...
def _toggle_group_favorite(item_id: str, add: bool) -> None:
    group_id = item_id.replace("group-", "")
    tag_id = get_or_create_tag(runtime.FAVORITE_TAG)
    if not tag_id:
        return
    group_res = stash_query(
...
def _toggle_performer_favorite(item_id: str, add: bool) -> None:
...
def _toggle_studio_favorite(item_id: str, add: bool) -> None:
...
async def endpoint_user_item_favorite(request):
    ...
    try:
        if item_id.startswith("scene-"):
            ...
            _toggle_scene_favorite(item_id, add=True)
        elif item_id.startswith("group-"):
            if not runtime.FAVORITE_TAG:
                return _favorite_response(item_id, True)
            _toggle_group_favorite(item_id, add=True)
        elif item_id.startswith("performer-") or item_id.startswith("person-"):
            _toggle_performer_favorite(item_id, add=True)
        elif item_id.startswith("studio-"):
            _toggle_studio_favorite(item_id, add=True)
    except Exception as e:
...
async def endpoint_user_item_unfavorite(request):
    ...
    try:
        if item_id.startswith("scene-"):
            ...
            _toggle_scene_favorite(item_id, add=False)
        elif item_id.startswith("group-"):
            if not runtime.FAVORITE_TAG:
                return _favorite_response(item_id, False)
            _toggle_group_favorite(item_id, add=False)
        elif item_id.startswith("performer-") or item_id.startswith("person-"):
            _toggle_performer_favorite(item_id, add=False)
        elif item_id.startswith("studio-"):
            _toggle_studio_favorite(item_id, add=False)
    except Exception as e:
