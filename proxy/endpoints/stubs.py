...
from proxy.stash.client import get_stash_session
...
async def endpoint_users_list(request):
    """`GET /Users` — configured users. Single-user proxy."""
...
async def endpoint_favicon(request):
    ...
    if _favicon_cache is None:
        try:
            ...
            resp = session.get(f"{runtime.STASH_URL.rstrip('/')}/favicon.ico", timeout=5)
