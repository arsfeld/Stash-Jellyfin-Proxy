...
from `stash_jellyfin_proxy.py`. It:
  ...
  8. Publishes all derived values to `proxy.runtime`

After this call, `proxy.runtime` is the single authoritative source of
truth for all config values. The callee can optionally read specific
values from `proxy.runtime` if it needs them locally.
"""
import os
import uuid

import proxy.runtime as runtime
...
from proxy.config.migration import run_config_migration, CURRENT_CONFIG_VERSION
...
def run_bootstrap(config_file: str, local_config_file: str) -> None:
    """Load config, apply overrides, publish to proxy.runtime.

    All side effects (print, file writes for SERVER_ID/ACCESS_TOKEN)
    happen here. After returning, proxy.runtime holds every config value.
    """
    # ---- Defaults ----
