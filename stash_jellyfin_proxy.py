    ...
    <script>
        ...
        async function fetchStatus() {
            try {
                ...
                document.getElementById('version').textContent = data.version || 'v6.00';
...
class RequestLoggingMiddleware:
    ...
    async def __call__(self, scope, receive, send):
        ...
        # Log request arrival immediately (before processing) so we can see in-flight requests
...
if __name__ == "__main__":
    ...
    logger.info(f"--- Stash-Jellyfin Proxy v6.00 ---")
